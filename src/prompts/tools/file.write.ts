...
export const prompt = ({ documents }: { documents: DocumentType[] }) => `You are an AI assistant with access to the following documents. Your task is to determine which document should be uploaded/save to the file system, based on the user's message.
...
3. Always provide your response in the following JSON format:
   {
     ...
     "name": "filename.md",
     "content": "Contents the user asked you to write/save to the file system. It may be your answer or a reference to the document in form of simply [[uuid]] without any additional text. But if you need to extract / rewrite / summarize / transform content from the document, you cannot use [[uuid]] placeholder unless available document is already in its final form."
   }
